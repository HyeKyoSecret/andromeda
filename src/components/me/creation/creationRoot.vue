<template>
    <mt-loadmore :top-method="loadTop" class="my-creation-content" ref="loadmore"
                 ...
                 infinite-scroll-distance = "10" :style="{'height': (scroll - 130) + 'px'}"
                 id="creationRoot">
        <router-link :to="item.path" tag='div' class="one-story" v-for="(item, index) in root" :key="item.name">
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      // document.getElementById('creationRoot').addEventListener('touchMove', function (event) {
      //   console.log('touchMove')
      //   event.stopPropagation()
      // }, false)
      // document.getElementById('creationRoot').addEventListener('scroll', function (event) {
      //   console.log('scroll')
    ...
    beforeRouteLeave (to, from, next) {
      from.meta.savedPosition = document.getElementById('creationRoot') ? document.getElementById('creationRoot').scrollTop : 0
      next()
    },
    activated () {
      if (this.root.length > 0) {
        Axios.post('/user/checkCreationRoot', {
          userId: this.$route.params.user,
          root: this.root[0].name
        }).then(response => {
          if (response.data.error) {
            ...
            if (response.data.userCheck) {
              setTimeout(() => {
                if (this.$route.meta.savedPosition > 0 && document.getElementById('creationRoot')) {
                  document.getElementById('creationRoot').scrollTop = this.$route.meta.savedPosition
                }
              }, 100)
            } else {
              this.root = []
